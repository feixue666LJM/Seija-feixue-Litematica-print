    ...
    public static boolean surfaceCheck(BlockPos pos, int c) {
        ...
        for (BlockPos blockPos : surface) {
            ...
            if ((!BlockReplaceUtils.INSTANCE.getScheState(blockPos).isSolid())
                ||(!BlockReplaceUtils.INSTANCE.getScheState(blockPos).isCollisionShapeFullBlock(Objects.requireNonNull(SchematicWorldHandler.getSchematicWorld()),blockPos))) return true;
        }
        return false;
    }
